...
def compute_and_save_flipped_data(
    X_train, y_train, X_test, y_test, clf, path_output_base, cols, flip_rate_range
):
    ...
    for rate in flip_rate_range:
        path_poison_data = "{}_randomlabelflip_svm_{:.2f}.csv".format(
        ...
        try:
            ...
            acc_test_poison = 0
        print(
            "Flip Rate [{:.2f}]% - Acc  Poisoned Train: {:.2f}%  Test Set: {:.2f}%".format(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.05,
        help="Spacing between values for flipping rates. Default=0.05",
    )
    parser.add_argument(
        "-m",
        "--max",
        type=float,
        default=0.41,
        help="End of interval for flipping rates. Default=0.41",
    )
