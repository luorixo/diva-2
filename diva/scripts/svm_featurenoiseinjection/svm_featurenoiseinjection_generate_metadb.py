...
def compute_and_save_noisy_data(
    X_train, y_train, X_test, y_test, clf, path_output_base, cols, noise_rate_range
):
    ...
    for rate in noise_rate_range:
        path_noisy_data = "{}_featurenoiseinjection_svm_{:.2f}.csv".format(
            path_output_base, np.round(rate, 2)
        )
        try:
            ...
            acc_test_noisy = 0
        print(
            "Noise Rate [{:.2f}%] - Acc  Noisy Train: {:.2f}%  Test Set: {:.2f}%".format(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.05,
        help="Spacing between values for noise rates. Default=0.05",
    )
    parser.add_argument(
        "-m",
        "--max",
        type=float,
        default=0.41,
        help="End of interval for noise rates. Default=0.41",
    )
    ...
    complexity_measures_df = extract_complexity_measures(
        "poisoned_data/feature_noise_svm"
    )
